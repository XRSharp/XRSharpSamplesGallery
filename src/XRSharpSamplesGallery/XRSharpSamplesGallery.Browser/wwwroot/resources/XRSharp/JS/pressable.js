// for hand tracking, see https://github.com/aframevr/aframe/blob/master/examples/showcase/hand-tracking/pressable.js
AFRAME.registerComponent('pressable', {
    schema: {
        pressDistance: { default: 0.014 },
        hoverDistance: { default: 0.020 }
    },

    init: function () {
        this.worldPosition = new THREE.Vector3();
        this.handEls = document.querySelectorAll('[hand-tracking-controls]');
        this.pressed = false;
        this.fingerOver = new Array(this.handEls.length);
    },

    tick: function () {
        var handEls = this.handEls;
        var handEl;
        var distance;
        for (var i = 0; i < handEls.length; i++) {
            handEl = handEls[i];
            distance = this.calculateFingerDistance(handEl.components['hand-tracking-controls'].indexTipPosition);
            if (distance < this.data.hoverDistance) {
                if (!this.fingerOver[i]) { this.el.emit('mouseenter', {}); }
                this.fingerOver[i] = true;
            } else if (this.fingerOver[i]) {
                this.el.emit('mouseleave', {});
                this.fingerOver[i] = false;
            }
            if (distance < this.data.pressDistance) {
                if (!this.pressed) { this.el.emit('mousedown', {}); }
                this.pressed = true;
                return;
            }
        }
        if (this.pressed) { this.el.emit('mouseup', {}); }
        this.pressed = false;
    },

    calculateFingerDistance: function (fingerPosition) {
        var el = this.el;
        var worldPosition = this.worldPosition;

        worldPosition.copy(el.object3D.position);
        el.object3D.parent.updateMatrixWorld();
        el.object3D.parent.localToWorld(worldPosition);

        return worldPosition.distanceTo(fingerPosition);
    }
});
